...
const InvoiceCard = ({ filter, invoices }) => {
          ...
          {filteredInvoices.map((invoice) => (
              <div onClick={() => navigate(`/invoice/${invoice.id}`)} key={invoice.id} className='flex dark:bg-slate-800  flex-wrap gap-4 justify-between items-center cursor-pointer w-full p-4  bg-slate-200 rounded-lg mb-4 hover:shadow-lg transition-shadow duration-300 hover:border-2  dark:hover:bg-slate-800'>
                      ...
                      <p className='text-slate-400 text-sm'>{invoice.name}</p>
                            <h1 className='text-white font-bold'>{invoice.currency} {invoice.amount}</h1>
                          <p className='text-slate-400 text-sm'>{invoice.date}</p>
                      <p className={`font-semibold flex justify-center items-center gap-2 ${invoice.status === "Paid" ? "text-green-500" : invoice.status === "Pending" ? "text-yellow-500" : "text-gray-500"}`}> <div className={`h-2 w-2 rounded-full ${invoice.status === "Paid" ? "bg-green-500" : invoice.status === "Pending" ? "bg-yellow-500" : "bg-gray-500"}  `}  ></div>
                        {invoice.status}</p>
