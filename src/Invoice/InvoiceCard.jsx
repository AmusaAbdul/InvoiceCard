...
const InvoiceCard = ({ filteredInvoice, invoice }) => {
    ...
    return (
        <div>
            {invoice.length === 0 ?
                ...
                <div className="p-3">
                    {filteredInvoice.map((inv, index) => (
                        <div onClick={() => navigate(`invoice/${inv.id}`)} key={index} className="flex justify-center items-center p-2 ">
                            <div className="flex w-full flex-wrap items-center gap-5 sm:gap-0 sm:flex-row justify-between p-3 sm:w-[80%] bg-blue-800 text-white">
                                <h1 className="font-bold">#{inv.id}</h1>
                                <p>{inv.issueDate}</p>
                                <h1>{inv.clientName}</h1>
                                <h1 className="font-bold">
                                    {formatter.format(
                                        (inv.items.reduce((sum, item) => sum + item.qty * item.price, 0)).toFixed(2)
                                    )}
                                </h1>
                                <p className={` p-2 flex gap-1 justify-center items-center shadow-md font-bold text-sm text-center ${inv.status === "Pending" ? "shadow-yellow-500 text-yellow-500" :
                                    inv.status === "Paid" ? "shadow-green-500 text-green-500" : "shadow-gray-500 text-gray-500"}`}>
                                    <span className={`h-2 w-2 rounded-full ${inv.status === "Pending" ? "bg-yellow-500 text-yellow-500" :
                                        inv.status === "Paid" ? "bg-green-500 text-green-500" : "bg-gray-500 text-gray-500"}`}>
                                    </span>{inv.status}
                                </p>
                                <button>&#9655;</button>
                            </div>
